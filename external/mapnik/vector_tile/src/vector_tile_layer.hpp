...
struct layer_builder_pbf {
  ...
  layer_builder_pbf(std::string const &name, std::uint32_t extent,
                    std::string &_layer_buffer)
      : keys(), values(), layer_buffer(_layer_buffer), empty(true),
        painted(false) {
  ...
  void make_painted() { painted = true; }

  void make_not_empty() { empty = false; }

  MAPNIK_VECTOR_INLINE protozero::pbf_writer
  add_feature(mapnik::feature_impl const &mapnik_feature,
...
public:
  tile_layer(mapnik::Map const &map, mapnik::layer const &lay,
             mapnik::box2d<double> const &tile_extent_bbox,
             std::uint32_t tile_size, std::int32_t buffer_size,
             double scale_factor, double scale_denom, int offset_x,
             int offset_y, mapnik::attributes const &vars)
      : valid_(true), ds_(lay.datasource()), target_proj_(map.srs(), true),
        source_proj_(lay.srs(), true), prj_trans_(target_proj_, source_proj_),
        buffer_(), name_(lay.name()), layer_extent_(calc_extent(tile_size)),
        target_buffered_extent_(calc_target_buffered_extent(
            tile_extent_bbox, buffer_size, lay, map)),
        source_buffered_extent_(calc_source_buffered_extent(lay)),
        query_(
            calc_query(scale_factor, scale_denom, tile_extent_bbox, lay, vars)),
        view_trans_(layer_extent_, layer_extent_, tile_extent_bbox, offset_x,
                    offset_y),
        empty_(true), painted_(false) {}

  tile_layer(tile_layer &&rhs)
      : valid_(std::move(rhs.valid_)), ds_(std::move(rhs.ds_)),
        target_proj_(std::move(rhs.target_proj_)),
        source_proj_(std::move(rhs.source_proj_)),
        prj_trans_(target_proj_, source_proj_), buffer_(std::move(rhs.buffer_)),
        ...
        query_(std::move(rhs.query_)), view_trans_(std::move(rhs.view_trans_)),
        empty_(std::move(rhs.empty_)), painted_(std::move(rhs.painted_)) {}
  ...
  std::uint32_t calc_extent(std::uint32_t layer_extent) {
    ...
    auto ds_extent = ds_->params().template get<mapnik::value_integer>(
        "vector_layer_extent");
    if (ds_extent) {
  ...
  mapnik::box2d<double> calc_target_buffered_extent(
      mapnik::box2d<double> const &tile_extent_bbox, std::int32_t buffer_size,
      mapnik::layer const &lay, mapnik::Map const &map) const {
    mapnik::box2d<double> ext(tile_extent_bbox);
    double scale = ext.width() / layer_extent_;
    double buffer_padding = 2.0 * scale;
    boost::optional<int> layer_buffer_size = lay.buffer_size();
    if (layer_buffer_size) // if layer overrides buffer size, use this value to
                           // compute buffered extent
    {
      if (!ds_ || ds_->type() == datasource::Vector) {
        buffer_padding *=
            (*layer_buffer_size) *
            (static_cast<double>(layer_extent_) / VT_LEGACY_IMAGE_SIZE);
      } else {
        buffer_padding *=
            (*layer_buffer_size) * (static_cast<double>(layer_extent_));
      }
    } else {
  ...
  mapnik::box2d<double> calc_source_buffered_extent(mapnik::layer const &lay) {
    ...
    if (!prj_trans_.forward(new_extent, PROJ_ENVELOPE_POINTS)) {
      if (!ds_ || ds_->type() != datasource::Vector) {
        throw std::runtime_error("vector_tile_processor: can not project "
                                 "target projection to an extent in the source "
                                 "projection, reproject source data prior to "
                                 "processing");
      } else {
  ...
  mapnik::query calc_query(double scale_factor, double scale_denom,
                           mapnik::box2d<double> const &tile_extent_bbox,
    ...
    if (scale_denom <= 0.0) {
      ...
      scale_denom =
          mapnik::scale_denominator(scale, target_proj_.is_geographic());
    }
    ...
    else if (prj_trans_.backward(query_extent, PROJ_ENVELOPE_POINTS) &&
             target_buffered_extent_.intersects(query_extent)) {
      query_extent.clip(target_buffered_extent_);
      // forward project layer extent back into native projection
      if (!prj_trans_.forward(query_extent, PROJ_ENVELOPE_POINTS)) {
        throw std::runtime_error("vector_tile_processor: query extent did not "
                                 "reproject back to source projection");
      }
    } else {
    ...
    if (!prj_trans_.equal()) {
      if (!prj_trans_.forward(unbuffered_query_extent, PROJ_ENVELOPE_POINTS)) {
        if (!ds_ || ds_->type() != datasource::Vector) {
          throw std::runtime_error("vector_tile_processor: can not project "
                                   "target projection to an extent in the "
                                   "source projection, reproject source data "
                                   "prior to processing");
        } else {
    ...
    double qw = unbuffered_query_extent.width() > 0
                    ? unbuffered_query_extent.width()
                    : 1.0;
    double qh = unbuffered_query_extent.height() > 0
                    ? unbuffered_query_extent.height()
                    : 1.0;
    if (!ds_ || ds_->type() == datasource::Vector) {
    ...
    if (ds_) {
      ...
      for (mapnik::attribute_descriptor const &desc :
           lay_desc.get_descriptors()) {
  ...
  mapnik::datasource_ptr get_ds() const { return ds_; }

  mapnik::featureset_ptr get_features() const { return ds_->features(query_); }

  mapnik::query const &get_query() const { return query_; }
  ...
  std::uint32_t layer_extent() const { return layer_extent_; }

  std::string const &name() const { return name_; }

  void name(std::string const &val) { name_ = val; }

  bool is_empty() const { return empty_; }

  bool is_painted() const { return painted_; }

  void make_painted() { painted_ = true; }

  std::string const &get_data() const { return buffer_; }

  std::string &get_data() { return buffer_; }
