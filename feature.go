...
type Identifier struct {
	m *C.struct__mapnik_identifier_t
}

func (id *Identifier) Free() {
	C.mapnik_identifier_free(id.m)
	id.m = nil
}

type Vaule struct {
	m *C.struct__mapnik_property_value_t
}

func (v *Vaule) Free() {
	C.mapnik_property_value_free(v.m)
	v.m = nil
}

type Property struct {
	m *C.struct__mapnik_property_map_t
}

func (v *Property) Free() {
	C.mapnik_property_map_free(v.m)
	v.m = nil
}

func NewProperty(m map[string]interface{}) *Property {
	pt := &Property{m: C.mapnik_property_map_new()}
	for k, v := range m {
		pt.SetVaule(k, v)
	}
	return pt
...
func NewFeature(geo *Geometry, id interface{}, prop map[string]interface{}) *Feature {
	var cid *C.struct__mapnik_identifier_t
	var cprop *C.struct__mapnik_property_map_t
	var lid *Identifier
	var lp *Property
	if id != nil {
		lid = NewIdentifier(id)
		cid = lid.m
		defer lid.Free()
	}
	if prop != nil {
	...
	return &Feature{m: C.feature_new(geo.m, cid, cprop)}
}
