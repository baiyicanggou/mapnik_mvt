...
int mapnik_map_load_string(mapnik_map_t *m, const char *str) {
  mapnik_map_reset_last_error(m);
  if (m && m->m) {
    try {
      mapnik::load_map_string(*m->m, str);
    } catch (std::exception const &ex) {
...
void mapnik_map_resize(mapnik_map_t *m, unsigned int width,
                       unsigned int height) {
  if (m && m->m) {
...
void mapnik_bbox_load(mapnik_bbox_t *b, double *bbox) {
  if (b) {
    bbox[0] = b->b.minx();
    bbox[1] = b->b.miny();
    bbox[2] = b->b.maxx();
...
mapnik_image_t *mapnik_map_render_to_image(mapnik_map_t *m, double scale,
                                           double scale_factor) {
  mapnik_map_reset_last_error(m);
...
int mapnik_map_render_to_file(mapnik_map_t *m, const char *filepath,
                              double scale, double scale_factor,
                              const char *format) {
  mapnik_map_reset_last_error(m);
  if (m && m->m) {
    try {
...
void mapnik_map_render_to_tile(mapnik_map_t *m, mapnik_tile_t *tile,
                               int buffer_size, int tile_size) {
  if (m && tile) {
    flywave::nik::make_image_tile(tile->data, *m->m, buffer_size, tile_size);
  }
}
...
mapnik_image_blob_t *mapnik_image_to_blob(mapnik_image_t *i,
                                          const char *format) {
...
mapnik_image_t *mapnik_image_from_raw(const uint8_t *raw, int width,
                                      int height) {
...
int mapnik_map_background(mapnik_map_t *m, uint8_t *r, uint8_t *g, uint8_t *b,
                          uint8_t *a) {
  if (m && m->m) {
...
void mapnik_map_set_background(mapnik_map_t *m, uint8_t r, uint8_t g, uint8_t b,
                               uint8_t a) {
  if (m && m->m) {
...
void mapnik_map_set_maximum_extent(mapnik_map_t *m, double x0, double y0,
                                   double x1, double y1) {
  if (m && m->m) {
...
void mapnik_map_layer_remove_all(mapnik_map_t *m) {
  if (m && m->m) {
    m->m->remove_all();
  }
}

void mapnik_map_layer_add_layer(mapnik_map_t *m, mapnik_layer_t *l) {
  if (m && m->m) {
    m->m->add_layer(l->layer);
  }
}

mapnik_layer_t *mapnik_layer(const char *name, const char *srs) {
  mapnik_layer_t *map = new mapnik_layer_t{mapnik::layer{name, srs}};
  return map;
}

void mapnik_layer_free(mapnik_layer_t *m) {
  if (m != NULL) {
    delete m;
  }
}

void mapnik_layer_add_style(mapnik_layer_t *m, const char *style) {
  if (m != NULL) {
    m->layer.add_style(style);
  }
}

void mapnik_layer_set_virtual_datasource(mapnik_layer_t *m, const char *name) {
  if (m != NULL) {
    m->layer.set_datasource(
        std::make_shared<flywave::nik::virtual_datasource>(name));
  }
}

void mapnik_layer_set_datasource(mapnik_layer_t *m, mapnik_datasource_t *ds) {
  if (m != NULL) {
    m->layer.set_datasource(ds->ptr);
  }
}

void mapnik_layer_set_name(mapnik_layer_t *m, const char *name) {
  if (m != NULL) {
    m->layer.set_name(name);
  }
}

void mapnik_layer_set_srs(mapnik_layer_t *m, const char *srs) {
  if (m != NULL) {
    m->layer.set_srs(srs);
  }
}

const char *mapnik_layer_get_name(mapnik_layer_t *m) {
  if (m != NULL) {
    return m->layer.name().c_str();
  }
  return NULL;
}

const char *mapnik_layer_get_srs(mapnik_layer_t *m) {
  if (m != NULL) {
    return m->layer.srs().c_str();
  }
  return NULL;
}

void mapnik_layer_set_min_zoom(mapnik_layer_t *m, double z) {
  if (m != NULL) {
    m->layer.set_minimum_scale_denominator(z);
  }
}

void mapnik_layer_set_max_zoom(mapnik_layer_t *m, double z) {
  if (m != NULL) {
    m->layer.set_maximum_scale_denominator(z);
  }
}

double mapnik_layer_get_min_zoom(mapnik_layer_t *m) {
  if (m != NULL) {
    return m->layer.minimum_scale_denominator();
  }
  return 0;
}

double mapnik_layer_get_max_zoom(mapnik_layer_t *m) {
  if (m != NULL) {
    return m->layer.maximum_scale_denominator();
  }
  return std::numeric_limits<double>::max();
}

void mapnik_layer_set_max_extent(mapnik_layer_t *m, mapnik_bbox_t *z) {
  if (m != NULL) {
    m->layer.set_maximum_extent(z->b);
  }
}

double *mapnik_layer_get_max_extent(mapnik_layer_t *m, double *bbox) {
  if (m != NULL && m->layer.maximum_extent()) {
    mapnik::box2d<double> b = m->layer.maximum_extent().value();
    bbox[0] = b.minx();
    bbox[1] = b.miny();
    bbox[2] = b.maxx();
    bbox[3] = b.maxy();
...
void mvt_map_render_to_tile(mapnik_map_t *m, mvt_tile_t *tile, int buffer_size,
                            int tile_size) {
  if (m && tile) {
    flywave::nik::make_vector_tile(tile->data, *m->m, buffer_size, tile_size);
  }
}

void utfgrid_map_render_to_tile(mapnik_map_t *m, utfgrid_tile_t *tile,
                                char *key, int buffer_size) {
  if (m && tile) {
    flywave::nik::make_utfgrid_tile(tile->data, *m->m, key, buffer_size);
  }
}
#ifdef __cplusplus
}
