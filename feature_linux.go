...
func NewIdentifier(t interface{}) *Identifier {
	switch v := t.(type) {
	case uint64:
		return &Identifier{m: C.mapnik_identifier_new_uint64(C.ulong(v))}
	case int64:
		return &Identifier{m: C.mapnik_identifier_new_int64(C.long(v))}
	case float64:
...
func NewVaule(t interface{}) *Vaule {
	...
	case uint64:
		return &Vaule{m: C.mapnik_property_value_new_uint64(C.ulong(v))}
	case int64:
		return &Vaule{m: C.mapnik_property_value_new_int64(C.long(v))}
	case float64:
...
func (p *Property) SetVaule(key string, t interface{}) {
	...
	case uint64:
		C.mapnik_property_map_set_uint64(p.m, keystr, C.ulong(v))
	case int64:
		C.mapnik_property_map_set_int64(p.m, keystr, C.long(v))
	case float64:
